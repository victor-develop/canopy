...
def build():
    ...
    tree.add_child("C1-1.0", "C1-3.0", "重试风暴", owner="A君", status="paused")
    tree.add_child("C1-2.0", "C1-4.0", "索引方案", owner="F君")
    tree.set_status("C1-4.0", "done")
...
def test_no_arg_is_one_rollup_line_per_root():
    ...
    assert "1 active" in lines[0] and "1 paused" in lines[0] and "1 done" in lines[0]
...
def test_depth_expands_that_many_levels_and_rolls_up_the_rest():
    ...
    assert "1 done" in child          # its own child collapsed into a rollup
...
def test_rollup_counts_locked_descendants():
    ...
    assert counts["active"] == 1 and counts["paused"] == 1 and counts["done"] == 1
